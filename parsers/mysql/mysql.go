...
var (
	// Current MySQL versions emit RFC3339 timestamps with either Z or a numeric
	// timezone offset (for example, 2026-08-24T10:30:03.356936+08:00).
	reTime = parsers.ExtRegexp{regexp.MustCompile("^# Time: (?P<time>[^ ]+) *$")}
	...
	// Depending on the server/version, Rows_affected and Bytes_sent may be
	// appended to the main Query_time line instead of being reported separately.
	reQueryStats = parsers.ExtRegexp{regexp.MustCompile("^# Query_time: (?P<queryTime>[0-9.]+) +Lock_time: (?P<lockTime>[0-9.]+) +Rows_sent: (?P<rowsSent>[0-9]+) +Rows_examined: (?P<rowsExamined>[0-9]+)(?: +Rows_affected: (?P<rowsAffected>[0-9]+))?(?: +Bytes_sent: (?P<queryBytesSent>[0-9]+))?.*$")}
	...
	// Older enhanced slow logs combine Bytes_sent and Tmp_* on one line, while
	// newer formats put Tmp_* on a dedicated line. Support both layouts.
	...
	reTmpStats  = parsers.ExtRegexp{regexp.MustCompile("^# Tmp_tables: +(?P<tmpTables>[0-9]+) +Tmp_disk_tables: +(?P<tmpDiskTables>[0-9]+) +Tmp_table_sizes: +(?P<tmpTableSizes>[0-9]+).*$")}
	reInnodbTrx = parsers.ExtRegexp{regexp.MustCompile("^# InnoDB_trx_id: (?P<trxId>[A-F0-9]+) *$")}
	// QC_Hit is emitted by some MariaDB/Percona variants but is absent from
	// standard MySQL output, so it must not be required to parse Full_scan.
...
func (p *Parser) handleEvent(ptp *perThreadParser, rawE []string) (
	...
	for _, line := range rawE {
		...
		if _, mg := reTime.FindStringSubmatchMap(line); mg != nil {
			if parsedTime, err := time.Parse(time.RFC3339Nano, mg["time"]); err == nil {
				timeFromComment = parsedTime
			}
			...
			if bytesSent, err := strconv.Atoi(mg["queryBytesSent"]); err == nil {
		...
		} else if _, mg := reTmpStats.FindStringSubmatchMap(line); mg != nil {
			query = ""
			if tmpTables, err := strconv.Atoi(mg["tmpTables"]); err == nil {
				sq[tmpTablesKey] = tmpTables
			}
			...
			if queryCacheHit, ok := mg["query_cache_hit"]; ok && queryCacheHit != "" {
				sq[queryCacheHitKey] = queryCacheHit == "Yes"
			}
