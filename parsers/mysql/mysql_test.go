...
func init() {
	...
	sqds = []slowQueryData{
		...
		{ /* 22: MySQL format without the optional QC_Hit field */
			rawE: []string{
...
func TestHandleEventModernMySQLSlowLog(t *testing.T) {
	p := &Parser{}
	ptp := &perThreadParser{normalizer: &normalizer.Parser{}}
	raw := []string{
		"# Time: 2026-08-24T10:30:03.356936+08:00",
		"# User@Host: ftf[ftf] @  [10.10.176.50]  Id: 2232231",
		"# Schema: ftf  Last_errno: 0  Killed: 0",
		"# Query_time: 0.221178  Lock_time: 0.220816  Rows_sent: 1  Rows_examined: 1  Rows_affected: 0  Bytes_sent: 102",
		"# Tmp_tables: 0  Tmp_disk_tables: 0  Tmp_table_sizes: 0",
		"# InnoDB_trx_id: 7FEF05B",
		"# Full_scan: No  Full_join: No  Tmp_table: No  Tmp_table_on_disk: No",
		"# Filesort: No  Filesort_on_disk: No  Merge_passes: 0",
		"# InnoDB_IO_r_ops: 0  InnoDB_IO_r_bytes: 0  InnoDB_IO_r_wait: 0.000000",
		"# InnoDB_rec_lock_wait: 0.220733  InnoDB_queue_wait: 0.000000",
		"# InnoDB_pages_distinct: 1",
		"SELECT 1;",
	}

	res, timestamp := p.handleEvent(ptp, raw)
	expected := map[string]interface{}{
		queryTimeKey: 0.221178, lockTimeKey: 0.220816,
		rowsSentKey: 1, rowsExaminedKey: 1, rowsAffectedKey: 0,
		bytesSentKey: 102, tmpTablesKey: 0, tmpDiskTablesKey: 0,
		tmpTableSizesKey: 0, fullScanKey: false, fullJoinKey: false,
		tmpTableKey: false, tmpTableOnDiskKey: false, recLockWaitKey: 0.220733,
	}
	for key, want := range expected {
		if !reflect.DeepEqual(res[key], want) {
			t.Errorf("key %s: expected %v, got %v", key, want, res[key])
		}
	}
	wantTime, _ := time.Parse(time.RFC3339Nano, "2026-08-24T10:30:03.356936+08:00")
	if !timestamp.Equal(wantTime) {
		t.Errorf("expected timestamp %v, got %v", wantTime, timestamp)
	}
}
