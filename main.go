...
// GlobalOptions has all the top level CLI flags that dbtail supports
type GlobalOptions struct {
	...
	ConfigFile string `short:"c" long:"config" description:"Config file for dbtail in INI format." no-ini:"true"`
	...
	Backfill         bool `long:"backfill" description:"Configure dbtail to ingest old data in order to backfill ClickHouse table. Sets the correct values for --backoff, --tail.read_from, and --tail.stop"`
...
func setVersionUserAgent(backfill bool, parserName string) {
	...
	libclick.UserAgentAddition = fmt.Sprintf("dbtail/%s (%s)", version, parserName)
}
...
func handleOtherModes(fp *flag.Parser, modes OtherModes) {
	if modes.Version {
		fmt.Println("DBtail version", version)
		os.Exit(0)
	}
	if modes.Help {
...
func usage() {
	fmt.Print(`
Usage: dbtail -p <parser> -f </path/to/logfile> -d <mydata> [optional arguments]
